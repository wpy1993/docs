...
1.0
新增了各种请求方式，post，put等； 增加请求头和相应头的概念
无状态、无连接

缺点：
每次请求，都需要进行一个tcp连接
队头堵塞，前一个请求响应到达后，才发起下一个
所以优化版1.1出现

1.1
新增connection字段，可以设置keep-alive true
管道化，可以一个tcp中同时发起多个请求，但是响应的顺序按照请求顺序返回。提升了并发性能
缓存增加了，cache-control、协商缓存
断点传输？ 实操用法不清楚，或者叫它分快传输编码，允许服务端将相应数据分快发送，客户端边接受边组装

缺点：
解析给予文本，所以难以切割为更小的快、效率一般


2.0
二进制分帧，将所有传输的信息分割为更小的消息和帧，并采用二进制格式的编码，提升传输效率
多路复用 同一域名下，所有的访问都走同一个tcp，http消息被分解为独立的帧，乱序发送，服务端重新组装
头部压缩，维护一个头部信息字典，减少头部信息传输占用的资源
